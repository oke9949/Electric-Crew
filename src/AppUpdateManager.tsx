...
async function fetchAvailableUpdate(): Promise<UpdateInfo | null> {
  ...
  try {
    ...
    const apk = (release.assets || []).find(asset =>
      ...
      String(asset.name || '').includes(version),
...
export default function AppUpdateManager() {
  ...
  const check = useCallback(async (force = false) => {
    if (!nativeAndroid || checking) return
    const now = Date.now()
  ...
  if (!nativeAndroid || !update || dismissed) return null

  const openUpdate = () => {
    const opened = window.open(update.downloadUrl, '_blank', 'noopener,noreferrer')
    if (!opened) window.location.assign(update.downloadUrl)
  }
  ...
  return (
    <aside className="app-update-banner" role="status" aria-live="polite">
      ...
      <div className="app-update-actions">
        <button className="btn primary" type="button" onClick={openUpdate}>
        ...
        <button className="icon-btn" type="button" title="Később" onClick={() => setDismissed(true)}>
          <X size={17}/>
        </button>
      </div>
    </aside>
